      ...
      <div className="flex flex-col md:flex-row gap-6 mt-12 sm:mt-18 w-10/12 mx-auto h-auto ">
        {data?.slice(0, 3).map((item, index) => (
          <React.Fragment key={item.id}>
            <div
              className={`border-[#E4E4E4] rounded-4xl border-4 xl:w-1/4 w-4/5 sm:w-3/5 md:w-[30%] mx-auto mt-12 relative h-[300px]
                hover:border-navyBlue hover: group
                
                `}
            >
              <div className="absolute top-[-50px] left-[50%] transform -translate-x-[50%]  bg-white">
                <img
                  src={item.pictureAddress || ax}
                  alt=""
                  className={`rounded-full border-4 border-[#E4E4E4] object-cover w-20 h-20 sm:w-24 sm:h-24 group-hover:border-navyBlue`}
                />
              </div>

              <h2 className="text-center mt-16 font-bold text-xl ">
                {item.fullName || "بینام"}
              </h2>
